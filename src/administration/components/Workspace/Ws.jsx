...
const Ws = () => {
  ...
  const getProfiles = async () => {
    ...
    if (authenticated()) {
      await client.service('ws').find({}).then(resp => {
        setState({...state, wslist: [
          ...resp.data.map(item => {
        ...
        <Box sx={{ bgcolor: '#f5f6fa',  borderRadius: '10px', padding: '10px' }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={11} container  alignItems="center">
              <Typography variant="h6">Liste des espace de travail (WorkSpaces)</Typography> 
            </Grid>
          </Grid>
          {/* End List head */}
   
          <Box sx={{height: '90%'}}>
            <WsList List={state.wslist} handleClickOpen={handleClickOpen} /> 
          </Box>
