...
const ActionBtn = ({row, handleClickOpen}) => {
  ...
  const options = {
    ...
    const supprimerWs = async () => {
      const result = await confirm("Etes-vous sûr de vouloir supprimer ?", options);
      if (result) {
        if (authenticated()) {
          setState({...state, progression: true})
          client.service("ws").remove(row._id).then(ws => {
            setState({
                ...state, 
                wslist: [...state.wslist.filter(item => item._id != ws._id) ], 
                progression: false,
                alertProps: {...state.alertProps, open: true, severity: 'error',  message: 'Espace supprimé avec success !' }
            })
          })
        } else {
...
const WsList = ({List, handleClickOpen}) => {

  const WsList = List && List.length ? [...List] : []
    ...
    <TableContainer component={Paper} sx={{ maxHeight: 500 }}>
      <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
        <TableHead>
          <TableRow>
            { WsList &&
              WsList.length && Object.keys(List[0]).map((name, index) => (
                <React.Fragment>
                {
            ...
            {WsList.length && (<TableCell align='center'> {"Actions"} </TableCell>)}
          </TableRow>
        </TableHead>
        <TableBody>
          {WsList.length && WsList.map((row, index) => (
