    ...
    const supprimerProfile = async () => {
      ...
      if (result) {
        if (authenticated()) {
          ...
          client.service("user-service").remove(row._id).then(service => {
            setState({
                ...state, 
                Servicelist: [...state.Servicelist.filter(item => item._id != service._id) ], 
                progression: false,
                alertProps: {...state.alertProps, open: true, severity: 'error',  message: 'Service supprimé avec success !' }
            })
          })
        } else {
...
const ServiceList = ({List, handleClickOpen}) => {
        ...
        <TableBody>
          {RolesList.length && RolesList.map((row, index) => (
            <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}  >
              ...
              <TableCell align="left">{row.abreviation}</TableCell>
